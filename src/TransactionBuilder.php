...
class TransactionBuilder
{
    ...
    */
    ...
    public function contractbalance($adres)
    {
        ...
        $jsonData = array_filter($jsonData['trc20_tokens'], function($value) { return $value['symbol']=='USDT'; });
        foreach($jsonData["trc20_tokens"] as $key =>$item)
        {
            ...
            if($balance_hex->compare(new BigInteger(0)))
            {
                $h = $balance_hex->value;

                $balance_hex->divide(new BigInteger(pow(10, $item['decimals'])));

                $trc20[]=array(
                    "name"=>$item["name"],
                    "symbol"=>$item["symbol"],
                    "balance"=>$balance_hex->toString(),
                    "value"=>$h,
